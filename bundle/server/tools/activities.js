...
import { ListActivitiesSchema, GetActivitySchema, CreateActivitySchema, UpdateActivitySchema, DeleteActivitySchema, } from "../schemas/activities.js";
...
export async function listActivities(params) {
    ...
    if (params.deal_id)
        queryParams.set("deal_id", String(params.deal_id));
    if (params.lead_id)
        queryParams.set("lead_id", params.lead_id);
    if (params.person_id)
    ...
    if (params.done !== undefined)
        queryParams.set("done", params.done ? "1" : "0");
...
export async function getActivity(params) {
    ...
    if (params.include_fields)
        queryParams.set("include_fields", params.include_fields);
    const response = await client.get(`/activities/${params.id}`, queryParams.toString() ? queryParams : undefined);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function updateActivity(params) {
    ...
    if (updateFields.due_date)
        body.due_date = updateFields.due_date;
    if (updateFields.due_time)
        body.due_time = updateFields.due_time;
    if (updateFields.duration)
        body.duration = updateFields.duration;
    if (updateFields.owner_id)
        body.owner_id = updateFields.owner_id;
    if (updateFields.deal_id)
        body.deal_id = updateFields.deal_id;
    if (updateFields.lead_id)
        body.lead_id = updateFields.lead_id;
    ...
    if (updateFields.project_id)
        body.project_id = updateFields.project_id;
    if (updateFields.note)
        body.note = updateFields.note;
    if (updateFields.done !== undefined)
        body.done = updateFields.done ? 1 : 0;
    if (updateFields.busy !== undefined)
        body.busy = updateFields.busy;
    if (updateFields.priority)
        body.priority = updateFields.priority;
    if (updateFields.participants)
        body.participants = updateFields.participants;
    if (updateFields.attendees)
        body.attendees = updateFields.attendees;
    if (updateFields.location)
        body.location = updateFields.location;
    const response = await client.patch(`/activities/${id}`, body);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function deleteActivity(params) {
    const client = getClient();
    const response = await client.delete(`/activities/${params.id}`);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
