...
import { ListDealsSchema, GetDealSchema, CreateDealSchema, UpdateDealSchema, SearchDealsSchema, DeleteDealSchema, } from "../schemas/deals.js";
...
export async function listDeals(params) {
    ...
    if (params.owner_id)
        queryParams.set("owner_id", String(params.owner_id));
    if (params.person_id)
        queryParams.set("person_id", String(params.person_id));
    if (params.org_id)
        queryParams.set("org_id", String(params.org_id));
    if (params.pipeline_id)
        queryParams.set("pipeline_id", String(params.pipeline_id));
    if (params.stage_id)
        queryParams.set("stage_id", String(params.stage_id));
    if (params.status)
        queryParams.set("status", params.status);
    if (params.updated_since)
        queryParams.set("updated_since", params.updated_since);
    if (params.updated_until)
        queryParams.set("updated_until", params.updated_until);
    if (params.sort_by)
        queryParams.set("sort_by", params.sort_by);
    if (params.sort_direction)
        queryParams.set("sort_direction", params.sort_direction);
    if (params.include_fields)
        queryParams.set("include_fields", params.include_fields);
    if (params.custom_fields)
        queryParams.set("custom_fields", params.custom_fields);
...
export async function getDeal(params) {
    ...
    if (params.custom_fields)
        queryParams.set("custom_fields", params.custom_fields);
    const response = await client.get(`/deals/${params.id}`, queryParams.toString() ? queryParams : undefined);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function createDeal(params) {
    ...
    if (params.probability !== undefined)
        body.probability = params.probability;
    if (params.visible_to)
        body.visible_to = params.visible_to;
    if (params.label_ids)
        body.label_ids = params.label_ids;
    if (params.add_time)
        body.add_time = params.add_time;
    if (params.custom_fields)
...
export async function updateDeal(params) {
    ...
    if (updateFields.person_id)
        body.person_id = updateFields.person_id;
    if (updateFields.org_id)
        body.org_id = updateFields.org_id;
    if (updateFields.pipeline_id)
        body.pipeline_id = updateFields.pipeline_id;
    if (updateFields.stage_id)
        body.stage_id = updateFields.stage_id;
    if (updateFields.status)
        body.status = updateFields.status;
    if (updateFields.expected_close_date)
        body.expected_close_date = updateFields.expected_close_date;
    if (updateFields.probability !== undefined)
        body.probability = updateFields.probability;
    if (updateFields.won_time)
        body.won_time = updateFields.won_time;
    if (updateFields.lost_time)
        body.lost_time = updateFields.lost_time;
    if (updateFields.lost_reason)
        body.lost_reason = updateFields.lost_reason;
    if (updateFields.label_ids)
        body.label_ids = updateFields.label_ids;
    if (updateFields.custom_fields)
        body.custom_fields = updateFields.custom_fields;
    const response = await client.patch(`/deals/${id}`, body);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function searchDeals(params) {
    ...
    if (params.person_id)
        queryParams.set("person_id", String(params.person_id));
    if (params.org_id)
        queryParams.set("organization_id", String(params.org_id));
    if (params.status)
        queryParams.set("status", params.status);
    ...
    // Search uses v1 API
    const response = await client.get("/itemSearch", queryParams, "v1");
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
 ...
 * Delete a deal
 */
export async function deleteDeal(params) {
    const client = getClient();
    const response = await client.delete(`/deals/${params.id}`);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
