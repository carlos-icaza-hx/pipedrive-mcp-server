...
import { ListNotesSchema, GetNoteSchema, CreateNoteSchema, UpdateNoteSchema, DeleteNoteSchema, } from "../schemas/notes.js";
...
export async function listNotes(params) {
    ...
    const response = await client.get("/notes", queryParams, "v1");
    if (!response.success) {
        return {
            content: [{
                    type: "text",
...
export async function getNote(params) {
    const client = getClient();
    const response = await client.get(`/notes/${params.id}`, undefined, "v1");
...
export async function createNote(params) {
    ...
    const response = await client.post("/notes", body, "v1");
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function updateNote(params) {
    ...
    const response = await client.put(`/notes/${id}`, body, "v1");
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function deleteNote(params) {
    const client = getClient();
    const response = await client.delete(`/notes/${params.id}`, "v1");
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
