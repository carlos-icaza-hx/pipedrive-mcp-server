...
import { ListOrganizationsSchema, GetOrganizationSchema, CreateOrganizationSchema, UpdateOrganizationSchema, SearchOrganizationsSchema, DeleteOrganizationSchema, } from "../schemas/organizations.js";
...
export async function listOrganizations(params) {
    ...
    const response = await client.get("/organizations", queryParams);
...
export async function getOrganization(params) {
    ...
    if (params.custom_fields)
        queryParams.set("custom_fields", params.custom_fields);
    const response = await client.get(`/organizations/${params.id}`, queryParams.toString() ? queryParams : undefined);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function updateOrganization(params) {
    ...
    const response = await client.patch(`/organizations/${id}`, body);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function deleteOrganization(params) {
    const client = getClient();
    const response = await client.delete(`/organizations/${params.id}`);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
