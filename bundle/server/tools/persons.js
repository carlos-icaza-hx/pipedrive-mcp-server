...
import { ListPersonsSchema, GetPersonSchema, CreatePersonSchema, UpdatePersonSchema, SearchPersonsSchema, DeletePersonSchema, } from "../schemas/persons.js";
...
export async function listPersons(params) {
    ...
    if (params.first_char)
        queryParams.set("first_char", params.first_char);
    ...
    const response = await client.get("/persons", queryParams);
...
export async function getPerson(params) {
    ...
    if (params.custom_fields)
        queryParams.set("custom_fields", params.custom_fields);
    const response = await client.get(`/persons/${params.id}`, queryParams.toString() ? queryParams : undefined);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function createPerson(params) {
    ...
    if (params.marketing_status)
        body.marketing_status = params.marketing_status;
    if (params.label_ids)
...
export async function updatePerson(params) {
    ...
    if (updateFields.visible_to)
        body.visible_to = updateFields.visible_to;
    if (updateFields.marketing_status)
        body.marketing_status = updateFields.marketing_status;
    ...
    const response = await client.patch(`/persons/${id}`, body);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
...
export async function searchPersons(params) {
    ...
    if (params.search_by_email !== undefined)
        queryParams.set("search_by_email", params.search_by_email ? "1" : "0");
    if (params.search_by_phone !== undefined)
        queryParams.set("search_by_phone", params.search_by_phone ? "1" : "0");
...
export async function deletePerson(params) {
    const client = getClient();
    const response = await client.delete(`/persons/${params.id}`);
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
