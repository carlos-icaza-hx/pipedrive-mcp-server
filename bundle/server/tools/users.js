...
import { ListUsersSchema, GetUserSchema, GetCurrentUserSchema, } from "../schemas/users.js";
import { formatErrorForMcp } from "../utils/errors.js";
/**
 * List all users
 */
export async function listUsers(_params) {
...
export async function getUser(params) {
    const client = getClient();
    const response = await client.get(`/users/${params.id}`, undefined, "v1");
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
 ...
 */
export async function getCurrentUser(_params) {
    const client = getClient();
    const response = await client.get("/users/me", undefined, "v1");
    if (!response.success || !response.data) {
        return {
            content: [{
                    type: "text",
